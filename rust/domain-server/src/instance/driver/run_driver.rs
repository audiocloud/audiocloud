...
pub async fn run_driver_server(instance_id: String,
  ...
  match config {
    | InstanceDriverConfig::USBHID(usb_hid) => {
      run_usb_hid_driver(instance_id, usb_hid, rx_cmd, tx_evt, scripting_engine).await?;
    }
    ...
    | InstanceDriverConfig::OSC(osc) => {
      run_osc_driver(instance_id, osc, rx_cmd, tx_evt, scripting_engine).await?;
    }
    ...
    | InstanceDriverConfig::SPI(_) => {
      error!("SPI driver is not implemented");
    }
