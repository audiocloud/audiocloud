...
use crate::audio_engine::{
    append_track, beautify_chunk, delete_track, set_track_chunk, ConnectionTemplate,
};
...
impl EngineFixedInstance {
    ...
    pub fn new(
        ...
        let (return_track, return_id) =
            append_track(&return_flow_id.clone().into(), project.context())?;
    ...
    pub fn get_return_state_chunk(
        &self,
        project: &EngineProjectTemplateSnapshot,
    ) -> anyhow::Result<String> {
        if self.use_reainsert() {
            Ok(beautify_chunk(
                ReaInsertReturnTemplate {
                    project,
                    instance: self,
                }
                .render()?,
    ...
    pub fn update_state_chunk(
        ...
        set_track_chunk(
            project.context(),
            self.send_track,
            &self.get_send_state_chunk(project)?,
        )?;
        set_track_chunk(
            project.context(),
            self.return_track,
    ...
    pub fn fill_peak_meters(&self, peaks: &mut HashMap<NodePadId, PadMetering>) {
        if let Some(routing) = self.routing {
            peaks.insert(
                self.send_pad_id.clone().into(),
                get_track_peak_meters(self.send_track, routing.send_count),
            );

            peaks.insert(
                self.return_pad_id.clone().into(),
