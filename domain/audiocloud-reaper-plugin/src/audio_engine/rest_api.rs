...
impl EngineClient {
    ...
    pub async fn play(&self, session_id: AppTaskId, play: RequestPlay) -> anyhow::Result<()> {
        self.request(move |tx| ReaperEngineCommand::Request((EngineCommand::Play { task_id: session_id, play }, tx)))
            .await
    }
...
async fn set_spec(client: web::Data<EngineClient>, path: web::Path<(AppId, TaskId)>, body: web::Json<SetSessionSpec>) -> impl Responder {
...
async fn do_render(client: web::Data<EngineClient>, path: web::Path<(AppId, TaskId)>, body: web::Json<RequestRender>) -> impl Responder {
    let (app_id, session_id) = path.into_inner();
    let id = AppTaskId::new(app_id, session_id);
    let body = body.into_inner();
...
async fn do_play(client: web::Data<EngineClient>, path: web::Path<(AppId, TaskId)>, body: web::Json<RequestPlay>) -> impl Responder {
    let (app_id, session_id) = path.into_inner();
    let id = AppTaskId::new(app_id, session_id);
    let body = body.into_inner();
...
async fn do_stop_play(client: web::Data<EngineClient>, path: web::Path<(AppId, TaskId, PlayId)>) -> impl Responder {
    ...
    Ok::<_, Error>(web::Json(client.stop_play(id, play_id).await.map_err(ErrorInternalServerError)?))
}
...
async fn do_stop_render(client: web::Data<EngineClient>, path: web::Path<(AppId, TaskId, RenderId)>) -> impl Responder {
    ...
    Ok::<_, Error>(web::Json(client.stop_render(id, render_id).await.map_err(ErrorInternalServerError)?))
}
