...
use libflac_sys::{
    ...
    FLAC__stream_encoder_set_channels, FLAC__stream_encoder_set_sample_rate,
...
impl FlacEncoder {
    ...
    pub fn new(
        ...
        if bits_per_sample > 16 {
            return Err(anyhow!(
                "The reference encoder only supports 16-bit encoding"
            ));
        }
        ...
        unsafe {
            ...
            assert_eq!(
                FLAC__stream_encoder_set_sample_rate(encoder, sample_rate as u32),
                1
            );
    ...
    pub fn process(
        &mut self,
        data: AudioBuf,
        output: &mut VecDeque<CompressedAudio>,
    ) -> anyhow::Result<()> {
        let converter = match self.bits_per_sample {
            ...
            i => {
                return Err(anyhow!(
                    "Only 16, 24 and 32 bits_per_sample supported, not {i}"
                ));
            }
        };
        ...
        if len > 0 {
            ...
            if !self.internals.buffer.is_empty() {
                output.push_back(CompressedAudio {
...
impl EncoderChain {
    ...
    pub fn process(&mut self, buf: &mut AudioBuffer<f64>, timeline: f64) -> anyhow::Result<()> {
        ...
        let buf = AudioBuf {
            timeline,
            stream: self.stream,
            channels: (0..inputs.len())
                .map(|i| Vec::from_iter(inputs.get(i).into_iter().copied()))
                .collect(),
        };
