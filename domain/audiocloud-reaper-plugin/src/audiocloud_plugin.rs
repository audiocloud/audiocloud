...
use crate::audio_engine::{
    PluginRegistry, ReaperEngine, ReaperEngineCommand, StreamingPluginCommand,
};
...
impl AudioCloudPluginActivation {
    ...
    pub(crate) fn process(
        ...
        if let Some(chain) = self.chain.as_mut() {
            chain.process(
                buf,
                Reaper::get().get_play_position_2_ex(self.context).get(),
            )?;
    ...
    fn dispatch_cmd(
        ...
        match cmd {
            StreamingPluginCommand::Play { context, play } => {
                ...
                self.chain = Some(EncoderChain::new(
                    play,
                    native_channels,
                    native_sample_rate,
                )?);
    ...
    fn make_drain(&self) -> impl Fn(PlayId, &mut VecDeque<CompressedAudio>) -> anyhow::Result<()> {
        ...
        move |play_id: PlayId, compressed: &mut VecDeque<CompressedAudio>| -> anyhow::Result<()> {
            while let Some(compressed_audio) = compressed.pop_front() {
                ...
                tx_engine.send(ReaperEngineCommand::Audio(
                    id.clone(),
                    play_id,
                    compressed_audio,
                ))?;
...
fn init_audio_engine(host: &HostCallback) -> ReaperSession {
    ...
    let ctx = PluginContext::from_vst_plugin(host, static_vst_plugin_context())
        .expect("REAPER PluginContext init success");
                ...
                if let Ok(cmd) = MsgPack.deserialize(&msg.data[..]) {
                    ...
                    if let Ok(_) = tx_cmd.send(ReaperEngineCommand::Request((cmd, tx))) {
                        thread::spawn(move || {
                            ...
                            let result = MsgPack
                                .serialize(&result)
                                .expect("Response serialization success");
    ...
    session
        .plugin_register_add_csurf_inst(Box::new(ReaperEngine::new(
            shared_media_root,
            tx_cmd,
            rx_cmd,
            tx_evt,
