...
pub async fn init(cfg: &DomainConfig, db: Db) -> anyhow::Result<()> {
    let models = match &cfg.models {
        ...
        DomainModelSource::Local { path } => {
            ...
            for model_path in globwalk::GlobWalkerBuilder::from_patterns(path, &["*.yaml", "*.yml"])
                .max_depth(4)
                .follow_links(true)
                .build()?
                .into_iter()
                .filter_map(Result::ok)
            {
                ...
                let (manufacturer, name) = model_file_stem.split_at(
                    model_file_stem
                        .find('_')
                        .ok_or_else(|| anyhow!("missing '_' character"))?,
                );
                let name = &name[1..];
                ...
                rv.insert(
                    ModelId::new(manufacturer.to_owned(), name.to_owned()),
                    model,
                );
            }
