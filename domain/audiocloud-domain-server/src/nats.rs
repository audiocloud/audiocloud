...
pub async fn publish<M: Serialize, C: Codec>(subject: &str, codec: C, message: M) -> anyhow::Result<()> {
    let connection = NATS_CONNECTION.get().ok_or_else(|| anyhow!("NATS_CONNECTION initialized"))?;
...
pub async fn request<R, C, S>(subject: S, codec: C, req: R) -> anyhow::Result<<R as Request>::Response>
    where R: Request,
          C: Codec,
          S: ToString
{
