...
use actix::{
    Actor, ActorFutureExt, AsyncContext, Context, ContextFutureSpawner, Handler, StreamHandler,
    WrapFuture,
};
...
use audiocloud_api::{
    FixedInstanceId, InstancePlayState, InstanceReports, Model, ModelCapability,
    PowerDistributorReports, Request, SerializableResult, Timestamped,
};

use crate::fixed_instances::values::merge_values;
use crate::fixed_instances::{
    get_instance_supervisor, NotifyFixedInstanceReports, NotifyInstancePowerChannelsChanged,
    NotifyInstanceState, SetDesiredPowerChannel, SetInstanceDesiredPlayState,
...
impl InstanceActor {
    ...
    fn on_instance_driver_reports(&mut self, reports: InstanceReports) {
        ...
        self.issue_system_async(NotifyFixedInstanceReports {
            instance_id: self.id.clone(),
            reports,
        });
    }
    ...
    fn on_instance_driver_connected(&mut self, ctx: &mut Context<InstanceActor>) {
        ...
        self.request_instance_driver(
            InstanceDriverCommand::SetParameters(self.parameters.clone()),
            ctx,
        );
    }

    fn on_instance_driver_play_state_changed(
        &mut self,
        current: InstancePlayState,
        media_pos: Option<f64>,
    ) {
...
impl InstanceActor {
    ...
    fn subscribe_instance_driver_events(&mut self, ctx: &mut Context<Self>) {
        ctx.add_stream(nats::subscribe_json::<InstanceDriverEvent>(
            self.id.driver_event_subject(),
        ));
    }
}
...
impl Handler<SetInstanceDesiredPlayState> for InstanceActor {
    ...
    fn handle(
        ...
        if let Some(media) = self.media.as_mut() {
            ...
            Err(DomainError::InstanceNotCapable {
                instance_id: self.id.clone(),
...
impl InstanceActor {
    ...
    fn request_instance_driver(
        &self,
        driver: InstanceDriverCommand,
        ctx: &mut <Self as Actor>::Context,
    ) {
        nats::request_json(self.instance_driver_cmd.to_string(), driver)
            .into_actor(self)
            .map(Self::on_instance_driver_response)
            .spawn(ctx);
    }

    fn on_instance_driver_response(
        response: anyhow::Result<<InstanceDriverCommand as Request>::Response>,
        actor: &mut Self,
        ctx: &mut <Self as Actor>::Context,
    ) {
