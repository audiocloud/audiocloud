...
impl SocketsSupervisor {
    #[instrument(skip_all)]
    pub fn on_socket_message_received(
        &mut self,
        message: SocketReceived,
        ctx: &mut <Self as Actor>::Context,
    ) {
        let (request, socket_id, use_json) = match message {
            SocketReceived::Bytes(socket_id, bytes) => {
                match MsgPack.deserialize::<DomainClientMessage>(bytes.as_ref()) {
        ...
        match request {
            DomainClientMessage::RequestModifyTaskSpec {
                ...
                let task_fut = get_tasks_supervisor().send(messages::ModifyTask {
                    modify_spec,
                    security,
                    task_id,
                    revision,
            ...
            DomainClientMessage::RequestAttachToTask {
                ...
                let response = DomainServerMessage::AttachToTaskResponse {
                    request_id,
            ...
            DomainClientMessage::RequestDetachFromTask {
                ...
                let response = DomainServerMessage::DetachFromTaskResponse {
                    request_id,
                    result: to_serializable(result),
                };
