...
impl Db {
    pub async fn delete_all_models(&self) -> anyhow::Result<()> {
        sqlx::query!(r#"DELETE FROM model WHERE true"#)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn set_model(&self, model_id: ModelId, model: Model) -> anyhow::Result<()> {
        ...
        sqlx::query!(
            r#"INSERT OR REPLACE INTO model (id, spec) VALUES (?, ?)"#,
            model_id,
            spec
        )
        .execute(&self.pool)
    ...
    pub async fn get_model(&self, model_id: &ModelId) -> anyhow::Result<Option<Model>> {
        let model_id = model_id.to_string();
        let result = sqlx::query!(r#"SELECT spec FROM model WHERE id=?"#, model_id)
