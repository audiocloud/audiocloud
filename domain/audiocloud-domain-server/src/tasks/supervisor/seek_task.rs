...
impl Handler<SeekTask> for TasksSupervisor {
    type Result = LocalBoxActorFuture<Self, DomainResult<TaskSought>>;
            ...
            task.send(msg)
                ...
                .map(move |res, actor, ctx| match res {
                    ...
                    Err(err) => Err(BadGateway {
                        error: format!("Task actor {task_id} failed to play: {err}"),
                    }),
                })
                .boxed_local()
        } else {
