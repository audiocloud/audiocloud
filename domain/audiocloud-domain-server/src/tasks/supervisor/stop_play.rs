...
impl Handler<StopPlayTask> for TasksSupervisor {
    type Result = LocalBoxActorFuture<Self, DomainResult<TaskPlayStopped>>;
    ...
    fn handle(&mut self, msg: StopPlayTask, ctx: &mut Self::Context) -> Self::Result {
        ...
        if let Some(task) = self
            .tasks
            .get(&msg.task_id)
            .and_then(|task| task.actor.as_ref())
        {
            ...
            task.send(msg)
                ...
                .map(move |res, actor, ctx| match res {
                    ...
                    Err(err) => Err(BadGateway {
                        error: format!("Task actor {task_id} failed to stop: {err}"),
                    }),
                })
                .boxed_local()
        } else {
