...
use crate::fixed_instances::{
    get_instance_supervisor, NotifyInstanceState, SetInstanceDesiredPlayState,
};
...
impl TaskFixedInstance {
    pub fn new(state_spec: NotifyInstanceState) -> Self {
        Self {
            state: Timestamped::new(state_spec),
        ...
        if let Some(media) = &self.state.value().play {
            if media.desired.value() != play {
                if self.tracker.should_retry() {
                    get_instance_supervisor().do_send(SetInstanceDesiredPlayState {
    ...
    let power_satisfied = i
        ...
        .map(|power| {
            power
                .actual
                .value()
                .satisfies(power.desired.value().clone())
        })
        .unwrap_or(true);

    let play_satisfied = i
        .state
        .value()
        .play
        .as_ref()
