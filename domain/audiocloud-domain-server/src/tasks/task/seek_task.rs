    ...
    fn handle(&mut self, msg: SeekTask, ctx: &mut Self::Context) -> Self::Result {
        match self.engine.get_actual_play_state() {
            TaskPlayState::Playing(playing) if &playing.play_id == &msg.seek.play_id => {
                let RequestSeek { play_id,
                                  segment,
                                  start_at,
                                  looping, } = msg.seek;

                let sought = TaskSought::Sought { task_id: { msg.task_id.clone() },
                                                  play_id: { play_id.clone() }, };

                let update = UpdateTaskPlay { play_id:  { msg.seek.play_id },
                                              mixer_id: { None },
                                              segment:  { Some(segment) },
                                              start_at: { Some(start_at) },
                                              looping:  { Some(looping) }, };

                self.engine.enqueue(EngineCommand::UpdatePlay { task_id: { msg.task_id },
                                                                update:  { update }, });
