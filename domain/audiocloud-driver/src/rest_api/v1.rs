...
async fn get_instances() -> impl Responder {
    ...
    let rv = get_driver_supervisor().send(command).await.map_err(ErrorInternalServerError)?;
...
async fn set_parameters(path: web::Path<(String, String, String)>, params: web::Json<serde_json::Value>) -> impl Responder {
    let instance_id = get_instance_id(path.into_inner());
...
async fn set_parameter(path: web::Path<(String, String, String, String)>, value: web::Json<serde_json::Value>) -> impl Responder {
