...
use crate::common::task::{
    ConnectionValues, DynamicInstanceNode, FixedInstanceNode, MediaChannels, MixerNode,
    NodeConnection, Task, TaskSpec, TimeSegment, TrackMedia, TrackNode, UpdateTaskTrackMedia,
};
use crate::common::time::Timestamped;
use crate::newtypes::{
    DynamicInstanceNodeId, FixedInstanceId, FixedInstanceNodeId, MediaObjectId, MixerNodeId,
    NodeConnectionId, SecureKey, TrackMediaId, TrackNodeId,
};
use crate::{
    json_schema_new_type, AppMediaObjectId, ChannelMask, InputPadId, OutputPadId, TaskNodeId,
    TaskSecurity,
};
...
impl TaskPlayState {
    ...
    pub fn satisfies(&self, desired: &DesiredTaskPlayState) -> bool {
        match (self, desired) {
            (Self::Playing(playing), DesiredTaskPlayState::Play(desired_playing)) => {
                playing == desired_playing
            }
            (Self::Rendering(rendering), DesiredTaskPlayState::Render(desired_rendering)) => {
...
impl Default for TaskState {
    fn default() -> Self {
        Self {
            play_state: Timestamped::new(TaskPlayState::Stopped),
...
impl TaskSpec {
    ...
    pub fn modify(&mut self, modify: ModifyTaskSpec) -> Result<(), ModifyTaskError> {
        match modify {
            ModifyTaskSpec::AddFixedInstance {
                fixed_id: mixer_id,
                spec: process,
            } => self.add_fixed_instance(mixer_id, process),
            ModifyTaskSpec::AddDynamicInstance {
                dynamic_id: mixer_id,
                spec: process,
            } => self.add_dynamic_instance(mixer_id, process),
            ModifyTaskSpec::AddMixer {
                mixer_id,
                spec: channels,
            } => self.add_mixer(mixer_id, channels),
            ...
            ModifyTaskSpec::DeleteDynamicInstance { dynamic_id } => {
    ...
    pub fn add_dynamic_instance(
        &mut self,
        dynamic_id: DynamicInstanceNodeId,
        dynamic: DynamicInstanceNode,
    ) -> Result<(), ModifyTaskError> {
        if self.dynamic.contains_key(&dynamic_id) {
            return Err(DynamicInstanceExists {
                node_id: dynamic_id,
            });
        }
    ...
    pub fn set_connection_parameter_values(
        &mut self,
        connection_id: NodeConnectionId,
        values: ConnectionValues,
    ) -> Result<(), ModifyTaskError> {
    ...
    pub fn set_fixed_instance_parameter_values(
        &mut self,
        ...
        let fixed = self
            .fixed
            .get_mut(&node_id)
            .ok_or(FixedInstanceDoesNotExist { node_id })?;
        // fixed.parameters.extend(parameters.into_iter());
        Ok(())
    }

    pub fn set_dynamic_instance_parameter_values(
        &mut self,
        ...
        let dynamic = self
            .dynamic
            .get_mut(&node_id)
            .ok_or(DynamicInstanceDoesNotExist { node_id })?;
    ...
    pub fn add_track_media(
        &mut self,
        track_id: TrackNodeId,
        media_id: TrackMediaId,
        spec: TrackMedia,
    ) -> Result<(), ModifyTaskError> {
        let track = self.tracks.get_mut(&track_id).ok_or(TrackDoesNotExist {
            node_id: track_id.clone(),
        })?;

        if track.media.contains_key(&media_id) {
            return Err(MediaDoesNotExist {
                node_id: track_id.clone(),
                media_id,
            });
        }
    ...
    pub fn delete_dynamic_instance(
        &mut self,
        node_id: DynamicInstanceNodeId,
    ) -> Result<(), ModifyTaskError> {
    ...
    pub fn update_track_media(
        ...
        let media = track.media.get_mut(&media_id).ok_or(MediaDoesNotExist {
            node_id: track_id.clone(),
            media_id,
        })?;
...
pub fn hashmap_changes<K: Hash + Eq + Clone, T: Clone + PartialEq>(
    existing: &HashMap<K, T>,
    new: &HashMap<K, T>,
) -> HashMapChanges<K, T> {
